...
class CcEnvironment(Environment):
    ...
    def agent_confirm_init(self, message: Message):
        ...
        if self.agents_ready == self.number_of_agents:
            self.agents_ready = 0
            self.log_message(f"All agents are ready!")
            #for id in range(self.number_of_agents):
            #    self.send_message("identity_decision", f"cc_agent.CcAgent {id+1}", None) 
            payload = payload = self.address_book.get_agents()
            self.send_message("start_election", "cc_ins.CcInstitution 1", payload)
